/* =============================================================
   ...
   Important shim: legacy code below uses bare `DATA`. We expose a
   getter so DATA references resolve to window.DATA at access time.
   The fmt object below is local to this file (different shape from
   the dashboard's fmt() function in format.js).
   ============================================================= */

// Shim — legacy code uses `DATA` not `window.DATA`
Object.defineProperty(window, 'DATA', {
  configurable: true,
  get: function() { return this._DATA; },
  set: function(v) { this._DATA = v; }
});
// Make `DATA` a global var alias to window.DATA via a property descriptor
// so existing code referencing `DATA` continues to work.
Object.defineProperty(globalThis, 'DATA', {
  configurable: true,
  get: function() { return window._DATA; },
  set: function(v) { window._DATA = v; }
});
...
factory capacity (lines, faster cadence, or premium yield).`;
...
function initCT() {
  ...
  DATA.stations.forEach(id => {
    ...
    row.innerHTML = `
      <div>${id}<span id="btag-${id}" class="bottleneck-tag" style="display:none;">SETS CADENCE</span></div>
